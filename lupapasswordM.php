...
if( isset($_POST["lupapasswordM"]) ) {
    $tanggallahir = $_POST["tanggallahir"];
    $email = $_POST["email"];

    $qry = $conn->query("SELECT * FROM mitraa WHERE tanggallahir ='$tanggallahir' AND email = '$email'");
    $cek = $qry->num_rows;
    
    if( $cek > 0) {
        $row = $qry->fetch_assoc();
        $_SESSION["lupapasswordM"] = true;
        $_SESSION["user_id"] = $row["id"];
        echo "
            <script type='text/javascript'>
                window.location='setpwm.php';
            </script>";
        exit;
    } else {
        $error = true;
    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <div class=btn-profil>
                <button type="submit" name="lupapasswordM">KIRIM</button>
            </div>
        </form>
        </div>
    </div>
